...
def calculate_tfidf_similarity(generated_response, expected_response):
    ...
    """
    vectorizer = TfidfVectorizer().fit_transform([generated_response, expected_response])
...
def calculate_llm_similarity(generated_response, expected_response, llm):
    ...
    messages = [
            ChatMessage(role="system", content="You are an expert assistant helping to compare text similarities."),
...
def main():
    ...
    for item in ground_truth:
        ...
        tfidf_similarity_score = calculate_tfidf_similarity(generated_response, expected_response)
        
        # Calculate similarity using LLM
        llm_similarity_score = calculate_llm_similarity(generated_response, expected_response, llm)
        ...
        results.append({
            ...
            "is_sufficient": bool(is_sufficient)  # Ensure the sufficiency flag is a bool
        })

    # Print summary of results
    sufficient_count = sum(result['is_sufficient'] for result in results)
    total_count = len(results)
    print(f"Results: {sufficient_count}/{total_count} responses are above the similarity threshold of 0.7")

    # Print the detailed results in a nicely readable format
    print(json.dumps(results, indent=4, ensure_ascii=False))

    # Output the results to a JSON file
    with open("evaluation/comparison_results.json", "w") as outfile:
